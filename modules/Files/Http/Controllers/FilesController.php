...
class FilesController extends BaseController
{
    ...
    public function uploadimagefromurl()
    {
        // getting all of the post data
        $input = Input::all();
        $file = file_get_contents($input['image']);
        $extension = pathinfo($input['image'], PATHINFO_EXTENSION);
        $size = strlen($file);
        $setting = $this->getFileSetting();
        $setting = json_decode($setting->content);
        $maximum_size = $setting->maximum_size * 1024 * 1024;
        ...
        if ($checkmimes == 0 || $size > $maximum_size) {
            return Response::json(array('status' => 0, 'url' => '', 'message' => trans('system.upload_fail')));
        } else {
            $time = time();
            $fileName = date('Y') . '/' . date('m') . '/';
            ...
            $name = $time . '-' . basename($input['image']);
            $fileName .= $name;
            $destinationPath = PATH_IMG . '/' . $fileName; // upload path
            $save = file_put_contents($destinationPath, $file);
            if ($save) {
                $user = $this->getUser();
                $objFile = new Files();
                $objFile->name = basename($input['image']);
                $objFile->size = $size;
                $objFile->mime = "";
                $objFile->url = 'files/' . $fileName;
                $objFile->folder_id = isset($input['folder_id']) ? $input['folder_id'] : 0;
